      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.editSheet, { backgroundColor: C.backgroundSecondary }]}>
          ...
          <View style={[styles.editFooter, { borderTopColor: C.separator }]}>
            <Pressable onPress={() => { if (!canSave) return; onSave({ name: name.trim(), category, color: colorResult?.colorLabel ?? item.color, colorHex: colorResult?.colorHex ?? item.colorHex, seasons, occasion: JSON.stringify(occasions), brand: brand.trim() || undefined, notes: notes.trim() || undefined }); }}
              disabled={!canSave}
