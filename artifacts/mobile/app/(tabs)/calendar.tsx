      ...
      {hasEntry && (
        <View style={styles.dotRow}>
          {entryColors.slice(0, 3).map((hex, i) => (
            <View
              key={i}
              style={[styles.dot, { backgroundColor: isSelected ? "rgba(255,255,255,0.8)" : hex }]}
            />
          ))}
        </View>
      )}
...
function OutfitPickerModal({
  ...
  const { logEntry, getEntry, removeEntry } = useCalendar();
  ...
  const handleSelect = async (outfitId: string) => {
  ...
  const handleRemove = () => {
    Alert.alert("Kaydı Sil", "Bu tarihteki kombin kaydı silinecek.", [
  ...
  const getOutfitColors = (outfitId: string) => {
    const outfit = outfits.find((o) => o.id === outfitId);
    if (!outfit) return [];
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <Pressable style={styles.modalOverlay} onPress={onClose}>
        <View
          style={[styles.modalSheet, { backgroundColor: C.backgroundSecondary }]}
        ...
        >
          ...
          <Text style={[styles.modalSubtitle, { color: C.textSecondary }]}>
            Hangi kombini giydini?
          </Text>

          {existing && (
            <View style={[styles.currentEntry, { backgroundColor: C.chip, borderColor: C.cardBorder }]}>
              <View style={styles.currentEntryLeft}>
                <Feather name="check-circle" size={16} color={C.tint} />
                <Text style={[styles.currentEntryText, { color: C.text }]}>
                  {existing.outfitName ?? "Kayıtlı kombin"}
                </Text>
              </View>
          ...
          {outfits.length === 0 ? (
            ...
            <FlatList
              ...
              renderItem={({ item: outfit }) => {
                const colors = getOutfitColors(outfit.id);
                const isCurrentlySelected = existing?.outfitId === outfit.id;
                return (
                  <Pressable
                    onPress={() => handleSelect(outfit.id)}
                    style={[
                      styles.outfitRow,
                      {
                        backgroundColor: isCurrentlySelected ? C.chip : C.card,
                        borderColor: isCurrentlySelected ? C.tint : C.cardBorder,
                      },
                    ]}
                  >
                    <View style={styles.outfitColorDots}>
                      {colors.slice(0, 4).map((hex, i) => (
                        <View
                          key={i}
                          style={[styles.outfitColorDot, { backgroundColor: hex }]}
                        />
                      ))}
                    </View>
    ...
    <View style={[styles.container, { backgroundColor: C.background }]}>
      ...
      >
        ...
        <View style={[styles.dayHeaders, { backgroundColor: C.card }]}>
          {DAYS_SHORT.map((d) => (
            <Text key={d} style={[styles.dayHeader, { color: C.textTertiary }]}>
              {d}
            </Text>
          ))}
        </View>
        ...
        <View style={styles.legendRow}>
          <View style={styles.legendItem}>
            ...
            <Text style={[styles.legendText, { color: C.textSecondary }]}>Kombin giyildi</Text>
          </View>
          <View style={styles.legendItem}>
              ...
              return (
                ...
                >
                  <View style={[styles.logDateBox, { backgroundColor: C.chip }]}>
                    ...
                    <Text style={[styles.logMonth, { color: C.textSecondary }]}>
                      {MONTHS_TR[m - 1].slice(0, 3)}
                    </Text>
                  </View>
                  <View style={styles.logInfo}>
                    <Text style={[styles.logOutfitName, { color: C.text }]} numberOfLines={1}>
                      {entry.outfitName ?? "Kombin"}
                    </Text>
